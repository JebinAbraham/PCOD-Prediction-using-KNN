...
query='SELECT * FROM hospitalplace ORDER BY dt DESC LIMIT 1'
mycursor.execute(query)
...
query = "Gynaec hospitals near "+ city 
...
hospitals={'name':[],'place':[]}

# Iterate over the search results
for i in range(len(y)):
    c=y[i]['name']
    hospitals['name'].append(c)
    hospitals['place'].append(city)
...
mycursor = mydb.cursor()
for i in range(0,10):
    query='INSERT INTO hospitals(name,place) VALUES (%s,%s)'

    values=[(hospitals['name'][i],hospitals['place'][i])]
