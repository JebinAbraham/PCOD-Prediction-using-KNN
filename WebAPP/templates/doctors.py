...
query='SELECT * FROM doctorplace ORDER BY dt DESC LIMIT 1'
mycursor.execute(query)
...
for i in range(len(y)):
    c=y[i]['name']
    doctors['name'].append(c)
    doctors['place'].append(city)
...
mycursor = mydb.cursor()
for i in range(0,10):
    query='INSERT INTO doctors(name,place) VALUES (%s,%s)'

    values=[(doctors['name'][i],doctors['place'][i])]
